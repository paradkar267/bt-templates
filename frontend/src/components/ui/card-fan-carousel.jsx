  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[300px] md:h-[400px]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-2xl bg-white dark:bg-black shadow-xl">
                ...
                <div className="absolute bottom-4 left-4 z-30">
                  <h3 className="text-white font-bold text-lg md:text-xl">{card.alt}</h3>
                </div>
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card absolute w-[200px] h-[280px] md:w-[260px] md:h-[360px] cursor-pointer">{image}</a>
            ) : (
              <div key={index} className="fan-card absolute w-[200px] h-[280px] md:w-[260px] md:h-[360px]">{image}</div>
            );
          })}
        </div>
      </div>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-8 md:mt-12 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-black/70 dark:bg-white/80 dark:bg-black/80 scale-[1.3]" : "bg-black/15 dark:bg-white/15"}`} />
            ))}
          </div>
